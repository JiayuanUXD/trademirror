...
6. 环境变量清单：`TURSO_DATABASE_URL` · `TURSO_AUTH_TOKEN` · `AUTH_SECRET` · `NEXTAUTH_SECRET` · `DEEPSEEK_API_KEY`（可选）· `TUSHARE_API_TOKEN`（可选，盘后分析）· `GOOGLE_CLIENT_ID`（可选）· `GOOGLE_CLIENT_SECRET`（可选）
