...
- feat: 盘后简报改为右侧抽屉打开（不再跳转页面）
- feat: 收盘后自动生成当日简报（无需手动点刷新）
- feat: 批量导入自动校验股票代码（用股票名反查东方财富，纠正 OCR 误识别）
- fix: 简报缓存接口 Tushare 不可用时 500 崩溃（降级返回最近缓存 + stale 标记）
- fix: `.env.local` 环境变量粘连导致 Tushare token 失效
- fix: 东田微重复持仓记录（301189 → 301183 代码纠正 + 重复决策卡作废）
- feat: 批量导入去重（已有相同股票代码/操作/价格/数量的决策卡自动跳过）
